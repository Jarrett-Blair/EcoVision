...
df = invert
alldata = ordertrainData[[1]]
allname = levels(as.factor(alldata$Order))
allname = allname[-7]
...
for(i in 2:ncol(hierarchy)){
  for(j in 1:length(allname)){
    if(which(hierarchylevels == df[which(df$Order == allname[j])[1], "Det_Level"]) < i){
      hierarchy[j,i] = as.character(df[which(df$Order == allname[j])[1],names(hierarchy)[i]])
    }
    else{
